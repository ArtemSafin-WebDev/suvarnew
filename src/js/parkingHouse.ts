...
export default function parkingHouse() {
    const elements = Array.from(document.querySelectorAll<HTMLElement>(".parking-house"));
    ...
    elements.forEach((element) => {
        ...
        const listItems = element.querySelectorAll<HTMLElement>(".parking-house__table-list-item");
        ...
        if (mediaQuery.matches) {
            ...
            listItems.forEach((listItem) => {
                const contentWrapper = listItem.querySelector<HTMLElement>(".parking-house__table-card-content-wrapper");
                ...
                const backButton = listItem.querySelector<HTMLButtonElement>(".parking-house__table-card-content-button-back");
        ...
        function handleAccordionClick(clickedElement: HTMLElement) {
            if (clickedElement.closest<HTMLElement>(".parking-house__table-list-item")?.classList.contains("active")) {
                clickedElement.closest<HTMLElement>(".parking-house__table-list-item")?.classList.remove("active");
                return;
            }

            const allItems = element.querySelectorAll<HTMLElement>(".parking-house__table-list-item");
            allItems.forEach(item => item.classList.remove("active"));

            const listItem = clickedElement.closest<HTMLElement>(".parking-house__table-list-item");
            if (listItem) {
                listItem.classList.add("active");
        ...
        function setupZoomAndDrag(container: HTMLElement) {
            const contentContainer = container.querySelector<HTMLDivElement>(".parking-house__image-draggable");
            const decreaseButton = container.querySelector<HTMLButtonElement>(".parking-house__decrease");
            const increaseButton = container.querySelector<HTMLButtonElement>(".parking-house__increase");
        ...
        const allFiltersModal = element.querySelector<HTMLElement>(".parking-house__all-filters-modal");
        const fixedShowFilters = element.querySelector<HTMLButtonElement>(".parking-house__fixed-show-filters");

        const sortSelects = Array.from(element.querySelectorAll<HTMLElement>(".parking-house__sort"));
        ...
        const moreButtons = element.querySelectorAll<HTMLButtonElement>(".parking-house__more");
        ...
        function openMoreModal(moreButton: HTMLButtonElement) {
            const listItem = moreButton.closest<HTMLElement>(".parking-house__table-list-item");
            if (!listItem) return;

            // Создаем попап
            const modal = document.createElement("div");
            modal.classList.add("parking-house__modal-more");

            const modalWrapper = document.createElement("div");
            modalWrapper.classList.add("parking-house__modal-more-wrapper");
            modal.appendChild(modalWrapper);


            // Добавляем контент в попап
            const title = listItem.querySelector<HTMLElement>(".parking-house__table-card-content-title");
            const price = listItem.querySelector<HTMLElement>(".parking-house__table-card-content-price");
            const options = listItem.querySelector<HTMLElement>(".parking-house__table-card-content-options");
            ...
            closeButton.classList.add("parking-house__modal-more-close", "js-close-modal-mores");
            ...
            const orderCallButton = listItem.querySelector<HTMLElement>(".parking-house__order-call");
            if (orderCallButton) {
