...
export default function boxroomsProjectPlan() {
    const houseItems = document.querySelectorAll<HTMLElement>(".boxrooms-project__plan-list-item");
    ...
    houseItems.forEach((houseItem) => {
        const floorSwitchItems = houseItem.querySelectorAll<HTMLElement>(".boxrooms-project__plan-card-floor-switch-item");
        const floors = houseItem.querySelectorAll<HTMLElement>(".boxrooms-project__plan-card-floor");
    ...
    function setupSortSelect(houseItem: HTMLElement) {
        const sortSelects = Array.from(houseItem.querySelectorAll<HTMLElement>(".boxrooms-project__plan-sort"));
        ...
        sortSelects.forEach((selectElement) => {
            ...
            if (selectedOption) {
                const sortBtnText = selectElement.querySelector<HTMLElement>('.boxrooms-project__plan-sort-btn-text');
                if (sortBtnText) {
    ...
    function setupZoomAndDrag(houseItem: HTMLElement) {
        const activeFloor = houseItem.querySelector<HTMLElement>(".boxrooms-project__plan-card-floor.active");
        if (!activeFloor) return;

        const zoomableContainer = activeFloor.querySelector<HTMLElement>(".boxrooms-project__zoom");
        const contentContainer = zoomableContainer?.querySelector<HTMLElement>(".boxrooms-project__image-draggable");
        const decreaseButton = activeFloor.querySelector<HTMLButtonElement>(".boxrooms-project__zoom-control.boxrooms-project__decrease");
        const increaseButton = activeFloor.querySelector<HTMLButtonElement>(".boxrooms-project__zoom-control.boxrooms-project__increase");
        ...
        if (mediaQuery.matches) {
            Draggable.create(contentContainer, {
                type: "x,y",
                edgeResistance: 0.65,
                bounds: zoomableContainer?.querySelector(".boxrooms-project__image-wrapper"),
                inertia: true,
                zIndexBoost: false,
            });
        }
                ...
                if (!isMobile) {
                    ...
                    marker.addEventListener("mouseenter", () => {
                        ...
                        if (!isPopupPositioned) {
                            ...
                            const contentWrapper = marker.closest<HTMLElement>(".boxrooms-project__plan-card-content-wrapper");
