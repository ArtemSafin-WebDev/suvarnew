...
export default function boxrooms() {
  const elements = Array.from(
      document.querySelectorAll<HTMLElement>(".boxrooms")
  );
  ...
  elements.forEach((element) => {
    ...
    const sortSelects = Array.from(
        element.querySelectorAll<HTMLElement>(".boxrooms__sort")
    );
    sortSelects.forEach((select) => new Select(select));

    const mainTabBtns = Array.from(
        element.querySelectorAll<HTMLElement>(".boxrooms-tabs__tabs-nav-link")
    );

    const mainTabItems = Array.from(
        element.querySelectorAll<HTMLElement>(".boxrooms-tabs__tabs-item")
    );
