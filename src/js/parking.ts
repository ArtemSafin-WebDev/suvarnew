...
export default function parking() {
  const elements = Array.from(
      document.querySelectorAll<HTMLElement>(".parking")
  );
  ...
  elements.forEach((element) => {
    ...
    const sortSelects = Array.from(
        element.querySelectorAll<HTMLElement>(".parking__sort")
    );
    sortSelects.forEach((select) => new Select(select));

    const mainTabBtns = Array.from(
        element.querySelectorAll<HTMLElement>(".parking-tabs__tabs-nav-link")
    );

    const mainTabItems = Array.from(
        element.querySelectorAll<HTMLElement>(".parking-tabs__tabs-item")
    );
    ...
    const mapElement = element.querySelector<HTMLElement>(".parking-projects__map");
    const rightColumn = element.querySelector<HTMLElement>(".parking-projects__right");
