...
async function initMap() {
  const ourProjects = document.querySelector(".boxrooms-projects");
  if (!ourProjects) return;
  const listElement = ourProjects.querySelector(".boxrooms-projects__list");
  if (!listElement) {
    return;
  }
  const mapElement = ourProjects.querySelector(".boxrooms-projects__map-inner");
  if (!mapElement) return;
  const mapRoot = ourProjects.querySelector(".boxrooms-projects__map");
  ...
  const createProjectListItem = (item, markerElement) => {
    ...
    listItem.className = "boxrooms-projects__list-item";
    listItem.setAttribute("data-title", item.title);

    let tagsHTML = "";
    item.tags.forEach((tag) => {
      tagsHTML += `
        <li class="boxrooms-projects__card-tags-list-item">
          <div class="boxrooms-projects__card-tag ${
          tag.dark ? "boxrooms-projects__card-tag--dark" : ""
      }">
            ${
          tag.icon
              ? `<svg width="15" height="16" aria-hidden="true" class="boxrooms-projects__card-tag-icon">
    ...
    item.actions.forEach((action) => {
      actionsHTML += `
        <li class="boxrooms-projects__card-options-item boxrooms-card-options-item">
          <svg width="15" height="16" aria-hidden="true" class="boxrooms-card-options-item__image">
            <use xlink:href="#${action.icon}"></use>
          </svg>
          <a href="${action.link}" class="boxrooms-card-options-item__link">${action.count.number} ${action.count.text}</a>
        </li>
      `;
    });
    ...
    listItem.innerHTML = `
      <div class="boxrooms-projects__card">
        ...
        <div class="boxrooms-projects__card-content">
          <div class="boxrooms-projects__card-row">
            <h3 class="boxrooms-projects__card-title">
              ${item.title}
            </h3>
          </div>
          <div class="boxrooms-projects__card-location">
            <svg width="14" height="14" aria-hidden="true">
              <use xlink:href="#location"></use>
            </svg>
            ${item.location}
          </div>
          <div class="boxrooms-projects__card-description">
            ${item.description}
          </div>
          <div class="boxrooms-projects__card-options">
            <ul class="boxrooms-projects__card-options-items">
              ${actionsHTML}
            </ul>
          </div>
        </div>
  ...
  const createProjectList = async () => {
    ...
    data.forEach((item) => {
      ...
      markerElement.className = "boxrooms-projects__map-marker";
      markerElement.innerHTML = `
        <div class="boxrooms-projects__map-marker-image-container">
          <img src="${item.image.src}" class="boxrooms-projects__map-marker-image" />
        </div>
        <div class="boxrooms-projects__map-marker-price-label" style="display: none;">
          <div class="boxrooms-projects__map-marker-title">${item.title}</div>
          <div class="boxrooms-projects__map-marker-price">${item.price}</div>
        </div>
      `;
  ...
  document.addEventListener("click", (event) => {
    if (
        ...
        !event.target.closest(".boxrooms-projects__list-item")
    ) {
